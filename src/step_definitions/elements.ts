...
defineSupportCode(({ When, Then }) => {
  ...
  When(/^I click the "([^"]*)" element$/, function(elementName) {
    return this.currentPage
      ...
      .catch(() => {
        return waitForCondition('elementToBeClickable', timeout)(this.currentPage[elementName]).then(() => {
      ...
      .catch(() => {
        console.warn(
          'Warning! Element was not clickable. We need use the WebDriver method to perform the click action.'
        );
        return browser
          .actions()
  ...
  When(/^I store the "([^"]*)" element text as "([^"]*)" variable$/, function(elementName, variable) {
    return this.currentPage.waitForVisibilityOf(elementName).then(async () => {
      ...
      if (elementTag === 'input' || elementTag === 'textarea') {
        return this.currentPage[elementName].getAttribute('value').then(value => {
          ...
          .each((element, index) => {
            ...
            for (const prop in hash) {
              if (hash.hasOwnProperty(prop)) {
                ...
                promises.push(
                  matchers.match(
                    element.element(self.currentPage[prop].locator()),
  ...
  Then(/^there are "([^"]*)" following elements for element "([^"]*)":$/, function(
    numberExpression,
    elementName,
    data
  ) {
  ...
  Then(/^there is no element "([^"]*)" with value "([^"]*)"$/, function(elementName, value) {
    ...
    return matchers
      .match(pageElement, variableStore.replaceTextVariables(value))
      .then(matcherResult => expect(matcherResult).not.toBe(true))
      .catch(() => Promise.resolve());
  });
    ...
    return this.currentPage.waitForVisibilityOf(containerName).then(() => {
      return this.currentPage[containerName]
        ...
        .then(firstElementText => {
          return self.currentPage[containerName].each(containerElement => {
            containerElement
              .element(self.currentPage[elementName].locator())
              .getText()
              .then(elementText => {
    ...
    return allElements
      .each(element => {
        hashedData.forEach(hash => {
          promises.push(
            matchers
              .match(element.element(self.currentPage[hash[0]].locator()), variableStore.replaceTextVariables(hash[1]))
              .catch(() => false)
          );
        });
      })
  ...
  Then(/^"([^"]*)" value on the "([^"]*)" list is sorted in "([^"]*)" order$/, function(
    ...
    return this.currentPage.waitForVisibilityOf(elementList).then(() => {
      return self.currentPage[elementList]
        .each(singleElement => {
  ...
  When(/^I drag "([^"]*)" element and drop over "([^"]*)" element$/, async function(elementDrag, elementDrop) {
    ...
    await browser
      .actions()
      .mouseMove(this.currentPage[elementDrag])
      .perform();
    await wait(200);
    await browser
      .actions()
    ...
    await browser
      .actions()
      .mouseMove(this.currentPage[elementDrop])
      .perform();
    await wait(200);
    await browser
      .actions()
