import { analyser } from './time-to-first-byte-analyser.helper';
import { expect } from 'chai';

const requests = [ { ttfb: 0, url: 'http://localhost:8080/' },
...
describe('Time to first byte analyser', () => {
  it('returns found slow request', () => {
    ...
    expect(analyser.analyseTTFB(requests, maxTiming)).to.deep.include({ ttfb: 2, url: 'http://localhost:8080/assets/kittens.jpg' });
  });

  it('returns empty array - no slow requests', () => {
    const maxTiming = 1000; // bigger than the highest request's TTFB value

    expect(analyser.analyseTTFB(requests, maxTiming)).to.deep.equal([]);
  });
});
