const textFields = require('./textFields');
...
describe('Text fields filter', () => {
  ...
  it('throws an error when value is not a regex or text matcher', () => {
    expect(() => textFields.filter(['some email content'], 'subject', 'some-value'))
  ...
  it('returns only emails matching given value using t:v:variableName', () => {
    ...
    const filteredEmails = textFields.filter(fakeEmails, 'subject', 't:v:someVariable');
  ...
  it('returns only emails matching given value using r:regexpName', () => {
    ...
    const filteredEmails = textFields.filter(fakeEmails, 'text_body', 'r:number');
